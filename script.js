...
const format =(acc, value)=>new Intl.NumberFormat(acc.locale,{style:'currency',currency:acc.currency}).format(value)
...
const calcDisplaySummary = function(acc){
  ...
  const interest = movs.filter(mov=>mov>0).map(deposit=> deposit* acc.interestRate /100).filter(int=>int >=1)
...
btnTransfer.addEventListener('click', function(e){
 ...
 if(reciver &&  amount > 0 && amount <= currentAccount.balance && currentAccount.userName !== reciver?.userName){
...
btnLoan.addEventListener('click' , function(e){
  ...
  if(amount>0 && currentAccount?.movements.filter(mov=> mov > 0)?.some(mov=> mov >= amount*.1)){
...
const accountIndex= accounts.findIndex(acc=>acc.userName === inputCloseUsername.value && acc.pin === +inputClosePin.value);
...
console.log(+'23') // cause Js when see + operator do type coercion an do convert the operands to number

 // parsing  => means Js will try to figer out the numbers from string

 console.log(Number.parseInt('30px' , 10)) //and should start with number
 console.log(Number.parseInt('e50', 10)) // will does'n work

 console.log(Number.parseInt('2.5rem' , 10))
 console.log(Number.parseFloat('2.5rem' , 10))
 
 console.log(parseFloat('2.5rem' , 10)) // this two func so-called global function and we don't have to call them on number but in modern JS we call them on the number object  cause the Number obj here provides somthing called a name spaces




//isNaN => to check if value it not a numkber
console.log(Number.isNaN(30)) // false is num
console.log(Number.isNaN('30')) // false regular value
console.log(Number.isNaN(+'30px')) // true is nan
...
console.log(Number.isFinite(23 / 0)) // false cause infinaty is not finit h 

console.log(Number.isInteger(23)) // true
console.log(Number.isInteger(23.0)) // also true because we always as decimals
console.log(Number.isInteger(23.5)) // false this actualy decimals
console.log(Number.isInteger(23 / 0)) // fasle cause infinaty is not finit 



console.log(`_______Math operations and rounding ____`)
// square root => as many other funcs the square root is part of the math namespace
console.log(Math.sqrt(25))
console.log(25 ** (1/2)) // the same result
console.log(25 ** (1/3)) // the same result for ( a cubic root) and the only way

// maximum  and minimum valu of a couple of values and it does type corcion but not parsing
console.log(Math.max(5,6,20,8, 10))
console.log(Math.max(5,6,'20',8, 10))
console.log(Math.max(5,6,'20px',8, 10)) // NaN

console.log(Math.min(5,6,'20',8, 10))

// and also constance on the math obj or thee math namespace
console.log(Math.PI * Number.parseFloat('10px')**2 )  //to get the area of circle with 10px radius 


Math.floor(Math.random()*6)+1 // random num between 1 --- 6

// to generat num between any num
const randomInt=(min, max)=> Math.floor(Math.random()*(max-min)+1)+min;
// 0 to 1 * (max - min)+1 => 0...((max - min)+1) + min => min ... max
...
// Rounding integers  and they all do type coercion
console.log(Math.trunc(23.2))

console.log(Math.round(23.2)) // to the nearst integer
...
// trunc and floor work the same on positive number only but on  negative num the don't 
// floor is more best than trunck cause it works the same in all sittions 
console.log(Math.trunc(-32.3))
console.log(Math.floor(-32.3))


// Rounding decimals
//toFixed returns string so we have to convert num if we need that
console.log((2.7).toFixed(0))
console.log((2.7).toFixed(1))
console.log(+(2.723).toFixed(3))
console.log((2.725).toFixed(2)) // it round 2.725 => 2.73
// and this works in a similar way than the string methods because num is primitive and its don't have methods so behind the secnce JS will do boxing to trnasform to a number object then call method on that object and when finished converted back to a primitive
*/
...
Array.from(document.querySelectorAll('.movements__row'), (elemnt , index)=>{
  if(index % 2 === 0) return elemnt.style.backgroundColor =  color?'orangered':''
})
};
...
const calcDaysPassed2 =(date1,date2)=> Math.round( Math.abs(date2-date1) /(1000*60*60*24));
...
console.log(navigator.language, new Intl.NumberFormat(navigator.language , options).format(num))// locale store from browser
...
const pizzaTimer = setTimeout((a,b)=> console.log(`Here is your pizza with ${a} and ${b} 🍕`),
3000,...ingrediants );
